...
const contentArray = cards.map((card, index) => {
  ...
  return (
    ...
    <div className="flex justify-between items-center">
          ...
          <button onClick={() => setDisplay(display + 1)} disabled={display === Math.ceil(testimonials.length / 3) - 1}><img src={arrowright} alt="arrow right" className="bg-grey-10 p-3 border border-solid border-grey-15 rounded-full aspect-square"/></button>
          </div>
        </div>
