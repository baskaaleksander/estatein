...
const LocationsComponent = () => {
    ...
    const contentArray = filteredLocations.map((quater, index) => {
        return (
            <LocationCard key={index} type={quater.type} name={quater.name} description={quater.description} badges={quater.badges} />
        )
    })
    ...
    return (
        <div className="text-white-0">
            <h2 className="text-5xl font-semibold laptop:text-4xl tablet:text-3xl mobile:text-2xl">Discover Our Office Locations</h2>
            <p className="text-lg laptop:text-base text-grey-60 pt-3 pb-20">Estatein is here to serve you across multiple locations. Whether you're looking to meet our team, discuss real estate opportunities, or simply drop by for a chat, we have offices conveniently located to serve your needs. Explore the categories below to find the Estatein office nearest to you</p>
            <div>
                <ButtonGroup buttons={["All","Regional", "International"]} onClick={handleClick}/>
                <div className="grid grid-cols-2 gap-8 py-20 navbar:grid-cols-1">
                    {contentArray}
                </div>
            </div>
        </div>
